...
class BaseRenderer:
    ...
    def __init__(self, **kwargs):
        ...
        self.horizontal_field_offset_class = kwargs.get(
            ...
            get_bootstrap_setting("horizontal_field_offset_class"),
        )
    ...
    def get_size_class(self, prefix):
        ...
        return (
            get_size_class(self.size, prefix=prefix)
...
class FieldRenderer(BaseRenderer):
    ...
    def __init__(self, field, **kwargs):
        ...
        self.help_text = (
            text_value(field.help_text) if self.show_help and field.help_text else ""
        )
        self.field_errors = [
            conditional_escape(text_value(error)) for error in field.errors
        ]

        self.placeholder = text_value(
            kwargs.get("placeholder", self.default_placeholder)
        )
    ...
    def add_widget_class_attrs(self, widget=None):
        ...
        if size_prefix:
            classes.append(
                get_size_class(self.size, prefix=size_prefix, skip=["xs", "md"])
            )

        if self.server_side_validation and self.can_widget_have_server_side_validation(
            widget
        ):
        ...
        for widget in widgets:
            ...
            if isinstance(widget, (RadioSelect, CheckboxSelectMultiple)):
                ...
                widget.template_name = (
                    "django_bootstrap5/widgets/clearable_file_input.html"
                )
    ...
    def render(self):
        ...
        if self.is_form_control_widget():
            ...
            if addon_before or addon_after:
                classes = "input-group"
                if (
                    self.server_side_validation
                    and self.get_server_side_validation_classes()
                ):
